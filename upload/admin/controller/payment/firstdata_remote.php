	...
	public function order() {
		if ($this->config->get('firstdata_remote_status')) {
			...
			if (!empty($firstdata_order)) {
				...
				$firstdata_order['total_formatted'] = $this->currency->format($firstdata_order['total'], $firstdata_order['currency_code'], 1, 'SSL');
				$firstdata_order['total_captured_formatted'] = $this->currency->format($firstdata_order['total_captured'], $firstdata_order['currency_code'], 1, 'SSL');
	...
	public function capture() {
		...
		if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '') {
			...
			if (strtoupper($capture_response['transaction_result']) == 'APPROVED') {
				...
				$json['data']['total_formatted'] = $this->currency->format($total_captured, $firstdata_order['currency_code'], 1, 'SSL');
				$json['error'] = false;
			} else {
				$json['error'] = true;
